...
function createHudPause({state,inner,setPlaying}){
  ...
  const position=()=>{button.style.top=`${Math.max(0,inner.clientWidth*.03958-14)}px`};
  new ResizeObserver(position).observe(inner);
  position();
  let press=null,lastTap=null,runtime=state.runtime,playing=state.playing,mode=state.appMode;
  ...
  function sync(){
    ...
    runtime=state.runtime;playing=state.playing;mode=state.appMode;
    button.disabled=!available();button.tabIndex=available()?0:-1;
    button.setAttribute('aria-label',state.playing?'暂停':'继续播放');
    button.setAttribute('data-hud-visible',String(state.hudVisible!==false));
  }
  function toggle(){clear();if(available())setPlaying(!state.playing);sync()}
  ...
  for(const type of ['pointerup','pointercancel'])window.addEventListener(type,e=>{
    pointers.delete(e.pointerId);if(type==='pointercancel')clear();
  },true);
  button.addEventListener('pointerdown',e=>{
    sync();e.stopPropagation();e.preventDefault();
    if(!available()||e.button!==0||e.isPrimary===false||pointers.size>1){clear();return}
    press={id:e.pointerId,x:e.clientX,y:e.clientY,at:performance.now(),touch:e.pointerType!=='mouse'};
    try{button.setPointerCapture(e.pointerId)}catch{clear()}
    button.focus({preventScroll:true});
  });
  button.addEventListener('pointermove',e=>{
    if(press?.id===e.pointerId&&Math.hypot(e.clientX-press.x,e.clientY-press.y)>8)clear();
  });
  button.addEventListener('pointerup',e=>{
    sync();e.preventDefault();e.stopPropagation();
    if(press?.id!==e.pointerId)return;
    const p=press,now=performance.now(),r=button.getBoundingClientRect();
    press=null;
    ...
    if(!p.touch||!state.playing){toggle();return}
    if(lastTap&&now-lastTap.at<=300&&Math.hypot(e.clientX-lastTap.x,e.clientY-lastTap.y)<=12){toggle();return}
  ...
  button.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(e.detail===0)toggle()});
  ...
  const reset=()=>{clear();pointers.clear()};
