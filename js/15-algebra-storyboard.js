...
function __algPackRGBA(c){return ((((Math.trunc(c[0])&255)<<24)|((Math.trunc(c[1])&255)<<16)|((Math.trunc(c[2])&255)<<8)|(Math.trunc(c[3])&255))>>>0)}
eventValue=function(ev,def,sec){
  ...
  const a=rgbaFromUint(fv),b=rgbaFromUint(tv),c=[0,0,0,0];
...
function __algStoryboardValue(rt,sb,key,sec){
  const tr=rt?.events?.get(BEARER_SB)?.get(sb.index)?.get(key),def=SB_DEFAULTS[key]??0;
  if(!tr||!tr.events?.length)return def;
...
function __algInstallSbEvaluator(rt){
  ...
  rt.sbValue=function(sb,key,sec){return __algStoryboardValue(rt,sb,key,sec)};
}
const __algPrecomputeBefore=precompute;
precompute=function(rt){__algPrecomputeBefore(rt);__algInstallSbEvaluator(rt)};
...
function __algRequestRenderOnce(){
  if(state?.playing||__algStoryboardRenderQueued)return;
  __algStoryboardRenderQueued=true;requestAnimationFrame(()=>{__algStoryboardRenderQueued=false;render()});
}
function __algBuiltinShape(kind){
  if(__algBuiltinCanvases.has(kind))return __algBuiltinCanvases.get(kind);
  const c=document.createElement('canvas'),g=c.getContext('2d',{alpha:true});
  if(kind==='builtin.line'){c.width=512;c.height=1;g.fillStyle='#fff';g.fillRect(0,0,512,1)}
  else{c.width=300;c.height=300;g.fillStyle='#fff';g.beginPath();if(kind==='builtin.round_rect'&&g.roundRect)g.roundRect(0,0,300,300,36);else g.rect(0,0,300,300);g.fill()}
  __algBuiltinCanvases.set(kind,c);return c;
}
function __algFileUrl(file){if(!(state.assetObjectUrls instanceof Map))state.assetObjectUrls=new Map();let u=state.assetObjectUrls.get(file);if(!u){u=URL.createObjectURL(file);state.assetObjectUrls.set(file,u)}return u}
function __algReady(d){return !!d&&((typeof HTMLCanvasElement!=='undefined'&&d instanceof HTMLCanvasElement)||(d.complete&&(d.naturalWidth||0)>0))}
function __algBuiltinTexture(data){
  const k={'builtin.tap':'tap','builtin.tap_double':'tap_double','builtin.extap':'extap','builtin.extap_double':'extap_double'}[data];if(!k)return null;
  let img=__algSbBuiltinImgs.get(k);if(!img){img=new Image();img.decoding='async';img.onload=()=>__algRequestRenderOnce();img.src=__ALG_SB_BUILTIN_NOTE_SRC[k];__algSbBuiltinImgs.set(k,img)}
...
storyImage=function(data){
  ...
  if(data!=='builtin.line'){const bt=__algBuiltinTexture(data);if(bt)return bt}
  let rec=storyCache.get(data);
  if(rec){const d=rec.drawable||rec.img;return __algReady(d)?d:null}
  let file=null;
  if(typeof window.__milResolveStoryboardFile==='function'){
    try{file=window.__milResolveStoryboardFile(data==='builtin.line'?'line.png':data)}catch{}
  }
  if(data==='builtin.line'&&!file)return __algBuiltinShape(data);
  if(!file){
    const d=__algStoryImageBefore(data),r=storyCache.get(data);
    if(r){const sw=r.img?.naturalWidth||d?.naturalWidth||d?.width||0,sh=r.img?.naturalHeight||d?.naturalHeight||d?.height||0;if(sw&&sh&&!r.sourceWidth){r.sourceWidth=sw;r.sourceHeight=sh}}
    return d;
  }
  const img=new Image();img.decoding='async';
  rec={img,drawable:null,failed:false,source:'',sourceWidth:0,sourceHeight:0};storyCache.set(data,rec);
  // 先保存原图几何尺寸；采样画布仅用于纹理，不能拿它重新计算故事板大小。
  img.onload=()=>{if(storyCache.get(data)!==rec)return;rec.sourceWidth=img.naturalWidth||0;rec.sourceHeight=img.naturalHeight||0;rec.drawable=window.__milSampleStoryboard(img);if(rec.drawable!==img)rec.img=null;__algRequestRenderOnce()};
  img.onerror=()=>{rec.failed=true};rec.source=__algFileUrl(file);img.src=rec.source;return null;
};
...
__milDrawRotTinted=function(img,cx,cy,w,h,deg,alpha,color){
  const sw=Number(img?.naturalWidth||img?.width)||0,sh=Number(img?.naturalHeight||img?.height)||0;if(!sw||!sh||![cx,cy,w,h,deg,alpha].every(Number.isFinite)||Math.abs(w)<1e-5||Math.abs(h)<1e-5)return;
  const sx=w<0?-1:1,sy=h<0?-1:1,aw=Math.abs(w),ah=Math.abs(h);ctx.save();ctx.globalAlpha*=alpha*(color[3]/255);ctx.translate(cx,cy);ctx.rotate(deg*Math.PI/180);ctx.scale(sx,sy);__milTintSlice(img,0,0,sw,sh,-aw/2,-ah/2,aw,ah,color);ctx.restore();
};
...
function __algUnityCanvasScale(w,h){return Math.max(w/800,h/600)}
function __algTextPx(w,h){return 300*.1125*__algUnityCanvasScale(w,h)}
...
window.__algebraStoryboardReviewSelfTest=function(){
  ...
  try{
    ...
    const landscape=window.__milStoryboardSampleSize(4096,2304),square=window.__milStoryboardSampleSize(8192,8192);
    ...
    const key='__algebra_geometry_regression__',old=storyCache.get(key),draw=__milDrawRotTinted,mode=state.appMode;
    try{
      const c=document.createElement('canvas');c.width=2560;c.height=1440;
      storyCache.set(key,{drawable:c,sourceWidth:4096,sourceHeight:2304});
      let actual=null;__milDrawRotTinted=(img,x,y,w,h)=>{actual={img,w,h}};state.appMode='play';
      const sb={index:-987654,type:0,layer:0,data:key},rt={storyboards:[sb],sbValue:(sb,k)=>k===COLOR?0xffffffff:([SIZE,SB_WIDTH,SB_HEIGHT,TRANSPARENCY].includes(k)?1:0)};
      drawStoryboardLayer(rt,0,0,1920,1080);
      ok(actual?.img===c&&Math.abs(actual.w-milX(4096,1920))<1e-6&&Math.abs(actual.h-milX(2304,1920))<1e-6,'sampled drawable preserves original geometry');
