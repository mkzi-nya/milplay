(()=>{
  ...
  shrink.id='playExpandedToggle';shrink.className='playExpandedToggle';shrink.type='button';shrink.textContent='⛶';shrink.title='退出放大';shrink.setAttribute('aria-label','退出放大');
  wrap.append(shrink);
  ...
  shrink.addEventListener('click',e=>{e.stopPropagation();setExpanded(false)});
  ...
  window.addEventListener('keydown',e=>{
    if(e.key==='Escape'&&wrap.classList.contains('playExpanded')){e.preventDefault();setExpanded(false)}
  },true);
