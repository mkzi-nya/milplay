...
function id(x){return document.getElementById(x)} function clamp(v,lo,hi){return Math.max(lo,Math.min(hi,v))} function fmt(v){return (Number.isFinite(v)?v:0).toFixed(3)} function safeName(s){return String(s||'chart').replace(/[\\/:*?"<>|]+/g,'_').slice(0,120)} function deepClone(o){return JSON.parse(JSON.stringify(o))} function setStatus(m,t=''){if(els.status){els.status.className='status'+(t?' '+t:'');els.status.textContent=m}if(!els.parseLog)return;const text=String(m||'');if(t==='ok'||(t!=='err'&&text==='')){els.parseLog.hidden=true;els.parseLog.value='';return}const stamp=new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'});const line='['+stamp+'] '+text;els.parseLog.value+=line+'\n';els.parseLog.hidden=false;els.parseLog.scrollTop=els.parseLog.scrollHeight}function __milResetParseLog(){if(els.parseLog){els.parseLog.value='';els.parseLog.hidden=true}}
