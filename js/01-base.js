...
const SB_DEFAULTS={0:0,1:0,2:0,3:1,4:0,6:0,7:0,10:1,11:1,14:-.5,15:-.5,16:.5,17:-.5,18:-.5,19:.5,20:.5,21:.5,22:0xffffffff};
...
function id(x){return document.getElementById(x)} function clamp(v,lo,hi){return Math.max(lo,Math.min(hi,v))} function fmt(v){return (Number.isFinite(v)?v:0).toFixed(3)} function safeName(s){return String(s||'chart').replace(/[\\/:*?"<>|]+/g,'_').slice(0,120)} function deepClone(o){return JSON.parse(JSON.stringify(o))} function __milLogStatus(m,t=''){if(!els.parseLog)return;const text=String(m||'');if(t==='ok'||(t!=='err'&&text==='')){els.parseLog.hidden=true;els.parseLog.value='';return}const stamp=new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'});els.parseLog.value+='['+stamp+'] '+text+'\n';els.parseLog.hidden=false;els.parseLog.scrollTop=els.parseLog.scrollHeight} function setStatus(m,t=''){if(els.status){els.status.className='status'+(t?' '+t:'');els.status.textContent=m}__milLogStatus(m,t)}function __milResetParseLog(){if(els.parseLog){els.parseLog.value='';els.parseLog.hidden=true}}
...
function updateUI(){const rt=state.runtime;if(!rt)return;els.playerCard?.classList.add('ready');const m=rt.meta||{},custom=state.externalChartMeta,title=(custom&&custom.title)||m.Title||m.name||state.fileName,customDifficulty=[custom&&custom.difficultyName,custom&&custom.difficultyLevel].filter(x=>x!=null&&String(x).trim()).join(' '),sub=custom?[customDifficulty,custom.illustrator&&'画师：'+custom.illustrator,custom.composer&&'曲师：'+custom.composer,custom.charter&&'谱师：'+custom.charter].filter(Boolean).join(' / '):[m.Difficulty||m.difficulty_name,m.Composer||m.music_artist,m.Beatmapper].filter(Boolean).join(' / ');if(els.songTitle)els.songTitle.textContent=title;if(els.songSub)els.songSub.textContent=sub||state.fileName;state.duration=rt.duration;if(els.timeSlider)els.timeSlider.max=String(rt.duration);if(els.durationLabel)els.durationLabel.textContent='/ '+fmt(rt.duration)+'s';updateStats();updateControls();setStatus(`${state.parseReport}\n${rt.notes.length} notes · ${rt.lineCount} lines · ${rt.storyboards.length} sb · ${RENDER_LAYER_ORDER}`,'ok')}function updateStats(){if(els.statNotes)els.statNotes.textContent=(state.runtime?.notes.filter(n=>n.type!==NOTE_FRACTURE).length||0)+' notes'}
...
async function inflateRaw(data){if('DecompressionStream' in window&&Blob.prototype.stream){try{return await new Response(new Blob([data]).stream().pipeThrough(new DecompressionStream('deflate-raw'))).arrayBuffer()}catch{}}if(window.fflate){const out=fflate.inflateSync(new Uint8Array(data));return out.buffer.slice(out.byteOffset,out.byteOffset+out.byteLength)}throw new Error('zip deflate 解压库未加载')}
