(()=>{
'use strict';
const stageWrap=els.stageWrap||document.getElementById('stageWrap');if(!stageWrap)return;
const overlay=document.createElement('div');overlay.id='gameResultOverlay';overlay.className='gameResultOverlay';overlay.setAttribute('aria-hidden','true');
...
const resultHeader=document.createElement('header');resultHeader.className='gameResultHeader';
const song=overlay.querySelector('.gameResultSong'),restart=overlay.querySelector('#gameResultRestart');
...
const backdrop=overlay.querySelector('.gameResultBackdrop'),cover=overlay.querySelector('#gameResultCover'),titleEl=overlay.querySelector('#gameResultTitle'),diffEl=overlay.querySelector('#gameResultDifficulty'),scoreEl=overlay.querySelector('#gameResultScore'),accEl=overlay.querySelector('#gameResultAcc'),perfectEl=overlay.querySelector('#gameResultPerfect'),goodEl=overlay.querySelector('#gameResultGood'),badEl=overlay.querySelector('#gameResultBad'),restartBtn=overlay.querySelector('#gameResultRestart');
let shownForRuntime=null,showQueued=false,dismissedForRuntime=null;
function resultMeta(){const m=state.runtime?.meta||state.chart?.meta||{};let difficulty=String(m.Difficulty||m.difficulty_name||'');if(m.DifficultyValue!=null&&!/\d\+?\s*$/.test(difficulty))difficulty+=' '+m.DifficultyValue;return{title:String(m.Title||m.name||state.fileName||'Unknown'),difficulty}}
function hideResult(suppress=false){const wasShowing=overlay.classList.contains('show');showQueued=false;overlay.classList.remove('show');overlay.setAttribute('aria-hidden','true');shownForRuntime=null;if(suppress&&wasShowing)dismissedForRuntime=state.runtime}
function showResult(){if(state.appMode!=='play'||!state.runtime||dismissedForRuntime===state.runtime)return;if(shownForRuntime===state.runtime&&overlay.classList.contains('show'))return;const r=window.__gpScoreBreakdown?.(true);if(!r)return;const c=r.counts||{},meta=resultMeta(),/* scoreMap 量级为 1e6/音符，换算成百分比需除以 1e4（1e6 -> 100%）。 */
...
window.__gpShowResult=showResult;window.__gpHideResult=hideResult;
restartBtn?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();dismissedForRuntime=null;hideResult(false);seek(0);setPlaying(true)},{capture:true});
/* Keep the overlay out of the way when replaying, seeking back, changing chart or mode. */
const baseSetPlaying=setPlaying;setPlaying=function(v){if(v)hideResult(false);return baseSetPlaying(v)};
const baseSeek=seek;seek=function(t){const n=Number(t)||0;if(n<(state.duration||0)-.001){dismissedForRuntime=null;hideResult(false)}return baseSeek(t)};
const basePrepare=prepare;prepare=function(...a){dismissedForRuntime=null;hideResult(false);return basePrepare(...a)};
if(typeof setAppMode==='function'){const baseMode=setAppMode;setAppMode=function(mode){if(mode!=='play')hideResult(false);return baseMode(mode)}}
...
const baseRender=render;render=function(){const ret=baseRender();if(state.appMode==='play'&&state.runtime&&dismissedForRuntime!==state.runtime&&!state.playing&&state.duration>0&&state.currentTime>=state.duration-.001&&!overlay.classList.contains('show')&&!showQueued){showQueued=true;queueMicrotask(()=>{showQueued=false;showResult()})}return ret};
els.audioPlayer?.addEventListener('ended',()=>{if(state.appMode!=='play'||!state.runtime)return;state.currentTime=state.duration||state.currentTime;render();queueMicrotask(showResult)});
window.__scoreResultSelfTest=function(){const f=window.calculateScore,fail=[];const ok=(x,m)=>{if(!x)fail.push(m)};if(typeof f!=='function')return{ok:false,failures:['calculateScore missing']};const ae=f(['e','e']);ok(ae.finalScore===1010000,'all Exact => 1010000');const ap=f(['p','p']);ok(ap.finalScore===1000000,'all Perfect => 1000000');const mix=f(['e','p','g','n','b','m']);ok(mix.counts.e===1&&mix.counts.p===1&&mix.counts.g===1&&mix.counts.n===1&&mix.counts.b===1&&mix.counts.m===1,'judge mapping');return{ok:!fail.length,failures:fail}};
})();
