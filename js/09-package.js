...
function __pkgBase(name){return __milBasename(String(name||''))}
function __pkgIsMeta(name){return /^meta\.json$/i.test(__pkgBase(name))}
function __pkgIsChart(name){return /\.(?:js|mjs|cjs|json)$/i.test(String(name||''))&&!/\.jsonl$/i.test(String(name||''))&&!__pkgIsMeta(name)}
let __pkgStoryboardPath='';
function __pkgIsStoryboardPath(name){const p=__milNormalizePath(name),root=__milNormalizePath(__pkgStoryboardPath),low=p.toLowerCase(),lowRoot=root.toLowerCase();return /(?:^|\/)(?:storyboard|storyboards)(?:\/|$)/i.test(p)||(!!root&&(low===lowRoot||low.indexOf(lowRoot+'/')===0))}
function __pkgIsAudio(name){return /\.(?:ogg|oga|opus|mp3|mp2|mpeg|wav|wave|flac|m4a|aac|aif|aiff|caf|weba|mp4|webm|mov)$/i.test(String(name||''))}
function __pkgIsAudioFile(f){return !!f&&(String(f.type||'').toLowerCase().startsWith('audio/')||__pkgIsAudio(f.name))}
function __pkgSorted(files){return [...files].sort((a,b)=>String(a.name).localeCompare(String(b.name),'en',{numeric:true,sensitivity:'base'}))}
function __pkgResolveStoryboardIn(files,ref){
  const all=[...(files||[])].filter(f=>f&&__pkgIsStoryboardPath(f.name)),pool=all.filter(f=>__milIsImageName(f.name));
  const base=__milBasename(ref),norm=__milNormalizePath(ref),baseLow=base.toLowerCase();
  ...
  /* Exact referenced image wins. Non-image Unity references (e.g. foo.asset) are
     ...
     never handed to <img>. */
  let m=pool.filter(f=>__milNormalizePath(f.name)===norm);if(m.length)return{matched:true,file:m.length===1?m[0]:null};
  m=pool.filter(f=>__milBasename(f.name)===base);if(m.length===1)return{matched:true,file:m[0]};
  if(m.length>1){const exact=m.filter(f=>__milNormalizePath(f.name).endsWith('/'+norm));return{matched:true,file:exact.length===1?exact[0]:null}}
  m=pool.filter(f=>__milBasename(f.name).toLowerCase()===baseLow);if(m.length===1)return{matched:true,file:m[0]};
  const targetStem=stem(base);m=pool.filter(f=>stem(f.name)===targetStem);if(m.length===1)return{matched:true,file:m[0]};
  if(m.length>1){const order=['.png','.avif','.webp','.jpg','.jpeg','.gif','.bmp','.svg'];for(const ext of order){const preferred=m.filter(f=>String(f.name).toLowerCase().endsWith(ext));if(preferred.length)return{matched:true,file:preferred.length===1?preferred[0]:null}}}
  /* A matching non-image resource means the basename exists, but is not drawable. */
  return{matched:all.some(f=>stem(f.name)===targetStem),file:null};
}
function __pkgCurrentStoryboardFiles(){return [...(state.assetFiles?.values?.()||[])].filter(f=>f&&__pkgIsStoryboardPath(f.name))}
function __pkgCurrentHasStoryboardDir(){return __pkgCurrentStoryboardFiles().length>0}
function __pkgResolveStoryboardFile(ref){
  ...
  const currentFiles=__pkgCurrentStoryboardFiles();if(currentFiles.length)return __pkgResolveStoryboardIn(currentFiles,ref).file;
  const batches=typeof __milEnsureUploadedAssetBatches==='function'?__milEnsureUploadedAssetBatches():[];
  for(let i=(batches?.length||0)-1;i>=0;i--){const r=__pkgResolveStoryboardIn(batches[i]?.files||[],ref);if(r.matched)return r.file}
  return null;
}
...
__milFindParseableCharts=async function(files){
  const filtered=(files||[]).filter(f=>!__pkgIsMeta(f?.name));const scan=await __pkgFindChartsBase(filtered);
  scan.matches.sort((a,b)=>{const aj=/\.(?:js|mjs|cjs)$/i.test(a.file?.name||'')?2:(/\.json$/i.test(a.file?.name||'')?1:0),bj=/\.(?:js|mjs|cjs)$/i.test(b.file?.name||'')?2:(/\.json$/i.test(b.file?.name||'')?1:0);return bj-aj||String(a.file?.name||'').localeCompare(String(b.file?.name||''),'en')});
  return scan;
};

function __pkgObjectUrl(file){
  if(!file)return null;if(!(state.assetObjectUrls instanceof Map))state.assetObjectUrls=new Map();const key=file;let u=state.assetObjectUrls.get(key);if(!u){u=URL.createObjectURL(file);state.assetObjectUrls.set(key,u)}return u;
}
/* Storyboard references get a dedicated same-basename lookup inside storyboard(s)/ first. */
const __pkgBuiltinStoryMap={'builtin.tap':'tap','builtin.tap_double':'tap_double','builtin.extap':'extap','builtin.extap_double':'extap_double'};
const __pkgBuiltinStoryShapes=new Map();
function __pkgBuiltinShape(kind){let c=__pkgBuiltinStoryShapes.get(kind);if(c)return c;c=document.createElement('canvas');const g=c.getContext('2d');if(kind==='builtin.line'){c.width=512;c.height=4;g.fillStyle='#fff';g.fillRect(0,1,512,2)}else{c.width=300;c.height=300;g.fillStyle='#fff';g.beginPath();if(kind==='builtin.round_rect'&&g.roundRect)g.roundRect(0,0,300,300,36);else g.rect(0,0,300,300);g.fill()}__pkgBuiltinStoryShapes.set(kind,c);return c}
function __pkgStoryDrawableReady(d){return !!d&&((d instanceof HTMLCanvasElement)||(d.complete&&d.naturalWidth>0))}
...
window.__milSampleStoryboard=function(img){
  const w=img.naturalWidth||img.width,h=img.naturalHeight||img.height,size=__pkgStorySampleSize(w,h);
  ...
  const g=c.getContext('2d',{alpha:true});g.imageSmoothingEnabled=true;g.imageSmoothingQuality='high';g.drawImage(img,0,0,c.width,c.height);return c;
};
storyImage=function(data){
  data=String(data||'');if(!data||data.startsWith('@i18n:'))return null;
...
__milPersistReferencedAssets=async function(chart,chartName,bgFile,mediaFile){
  ...
  for(const sb of chart?.storyboardObjects||chart?.storyboards||[]){if(int(sb?.type,0)===0){const f=__pkgResolveStoryboardFile(sb.data)||(!__pkgCurrentHasStoryboardDir()?__milResolveAssetFile(sb.data):null);if(f)refs.push(f)}}
...
async function __pkgLoadFiles(fileList){
  ...
  if(metaPackage.recognized&&metaPackage.valid){
    ...
    if(records.length){
      ...
      __pkgActiveMetaPackage={records,files,baseReport:metaReport.join('\n')};__pkgSetMetaChooser(records);
      const select=id('packageChartSelect');if(select)select.value='0';
      await __pkgApplyMetaRecord(records[0]);return;
    }
  }
   ...
   if(chartFile&&parsed){__pkgActiveMetaPackage=null;__pkgStoryboardPath='';state.externalChartMeta=null;__pkgSetMetaChooser(null);makeRuntime(parsed.chart,chartFile.name)}
   const uploadedBatch=typeof __milRegisterUploadedAssetBatch==='function'?__milRegisterUploadedAssetBatch(files):null;
   if(chartFile&&parsed){
     setPlaying(false);
    __milIndexPackageAssets(files,chartFile.name);const chart=parsed.chart,meta=chart?.meta||{};if(Array.isArray(chart?._warnings))report.push(...chart._warnings.map(x=>'警告：'+x));
    const illustrationRef=meta.IllustrationFile??meta.illustrationFile??meta.background,audioRef=meta.AudioFile??meta.audioFile??meta.music;
    const imgFile=__pkgPickBackground(files,illustrationRef),mediaFile=__pkgPickAudio(files,audioRef),sbFiles=files.filter(f=>__pkgIsStoryboardPath(f.name));
    const hasRequestedShape=files.some(f=>__pkgIsChart(f.name))&&!!imgFile&&!!mediaFile&&sbFiles.length>0;if(hasRequestedShape)report.push('已按 Milthm ZIP 结构识别：谱面 / 背景 / 音频 / storyboard(s)');
