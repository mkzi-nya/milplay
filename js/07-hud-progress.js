...
function createHudProgress({state,stage,inner,seek}){
  ...
  function sync(){
    ...
    const enabled=available(),duration=Math.max(0,state.duration||0);
  ...
  function valid(){
    if(!available()||!drag||drag.runtime!==state.runtime||drag.duration!==state.duration||drag.time!==state.currentTime){stop();return false}
    return true;
  }
  function jump(t){
    seeking=true;
    try{seek(Math.max(0,Math.min(state.duration,t)))}finally{seeking=false}
  ...
  function point(e){
    ...
    const m=transform==='none'?{a:1,b:0}:new DOMMatrixReadOnly(transform);
  ...
  slider.addEventListener('pointerdown',e=>{
    ...
    drag={id:e.pointerId,runtime:state.runtime,duration:state.duration,time:state.currentTime};
    try{slider.setPointerCapture(e.pointerId)}catch{stop();return}
    slider.focus({preventScroll:true});
    point(e);
  });
  slider.addEventListener('pointermove',e=>{
    if(!drag||e.pointerId!==drag.id||!valid())return;
    e.preventDefault();e.stopPropagation();point(e);
  });
  slider.addEventListener('pointerup',e=>{
    if(!drag||e.pointerId!==drag.id||!valid())return;
    e.preventDefault();e.stopPropagation();point(e);stop();
  });
  for(const type of ['pointercancel','lostpointercapture'])slider.addEventListener(type,e=>{if(drag?.id===e.pointerId)stop()});
  slider.addEventListener('keydown',e=>{
    if(!available()||e.altKey||e.ctrlKey||e.metaKey)return;
    const t=state.currentTime||0,d=state.duration;
    const targets={ArrowLeft:t-1,ArrowDown:t-1,ArrowRight:t+1,ArrowUp:t+1,PageDown:t-d/10,PageUp:t+d/10,Home:0,End:d};
    if(!Object.prototype.hasOwnProperty.call(targets,e.key))return;
    e.preventDefault();e.stopPropagation();stop();jump(targets[e.key]);
  });
