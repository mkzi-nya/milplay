...
function __pluEffectLinePoint(n,sec,w,h,rt){
  if(!n||!rt)return null;
  const source=n.__pluEffectSource||n,st=transformLine(rt,source.lineIdx,sec,w,h),f=__pluNoteFrame(rt,source,sec,st,w,h);
  if(!f)return null;
  const point=applyLineWorld(st,w,h,f.baseX,0);
  return{x:point.x,y:point.y,scale:f.scale,alpha:f.alpha};
}
...
function __pluAnchorEffectNote(n,sec,w=els.stage.width,h=els.stage.height,rt=state.runtime){
  ...
  const anchor={rt,time:sec,w,h,...point};
...
__pluDrawHitRing=function(rt,n,sec,st,w,h){
  ...
  const anchor=__pluAnchorEffectNote(n,n.startSec,w,h,rt);if(!anchor)return;
  const p=(sec-n.startSec)/.5,lineHeadBase=(w+h)*.0223*(state.noteScale||1),size=lineHeadBase*4.632*(1-(1-p)**3)*anchor.scale;if(!(size>0))return;
