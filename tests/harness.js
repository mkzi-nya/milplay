...
function createHarness(root,opts={}){
  ...
  const ids=new Set([...html.matchAll(/\bid="([^"]+)"/g)].map(m=>m[1])),nodes=new Map(),messages=new Set(),windowEvents=new Map(),images=[],blobs=[],revoked=[];
  const noop=()=>{},raf=[],storage=opts.storage||new Map();
