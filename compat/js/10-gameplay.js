(() => {
  ...
  function gpIsPlay() {
    return state.appMode === 'play' && !!state.runtime;
  }
  ...
  function gpEntry(n) {
    ...
    if (!e) {
      e = {
        judgeState: 'Miss',
        judgeTime: n.startSec + Math.max(GP_WINDOWS.Bad * 2, .14),
        judgeHited: false,
        judgeIsMiss: false,
        judgeMissTime: n.endSec,
        headJudged: false,
        dragPrejudge: false,
        prejudgeTime: 0,
        judgeHoldLastCheck: 0,
        prejudgeTouches: new Set(),
        gaveEndCombo: false,
        judgeIsGood: false,
        hitParticleOffset: 0,
        seekHeld: false
      };
      gp.entries.set(n.key, e);
    }
    return e;
  }
  function gpSubmitAcc(v) {
    gp.accSum += Number(v) || 0;
    gp.fullAcc++;
  }
  function gpAddCombo() {
    gp.combo++;
    gp.maxCombo = Math.max(gp.maxCombo, gp.combo);
  }
  function gpRemoveCombo() {
    gp.combo = 0;
  }
  function gpCut(name) {
  ...
  function gpJudgeState(n, offset) {
    ...
    let s = a < GP_WINDOWS.Exact ? 'Exact' : a < GP_WINDOWS.Perfect ? 'Perfect' : a < GP_WINDOWS.Great ? 'Great' : a < GP_WINDOWS.Good ? 'Good' : a < GP_WINDOWS.Bad ? 'Bad' : 'Miss';
  ...
  function gpOrdinaryComboTimes(rt = state.runtime) {
    const out = [];
    for (const n of (rt == null ? void 0 : rt.notes) || []) {
      if (!gpOrdinary(n)) continue;
      out.push(n.startSec);
      if (n.isHold) out.push(n.endSec);
    }
    out.sort((a, b) => a - b);
    return out;
  }
  function gpOrdinaryComboAt(sec) {
    const a = gp.comboTimes || [];
    let lo = 0,
      hi = a.length;
    while (lo < hi) {
      const m = lo + hi >> 1;
      if (a[m] <= sec) lo = m + 1;else hi = m;
    }
    return lo;
  }
  function gpFresh(rt = state.runtime) {
    ...
    gp.allCombo = gp.comboTimes.length;
    gp.touchHoldEnd = 0;
    gp.holdLasts = -1e9;
    gp.lastFixed = -GP_STEP;
    gp.indicatorBalls = [];
    gp.effectKeys = new Set();
    gp.noteByKey = new Map();
    gp.timeBuckets = new Map();
    gp.longNotes = [];
    const span = GP_WINDOWS.Bad * 2,
      bs = .25;
    gp.bucketSec = bs;
    for (const n of (rt == null ? void 0 : rt.notes) || []) {
      if (!gpOrdinary(n)) continue;
      gp.noteByKey.set(n.key, n);
      const a = Math.max(0, n.startSec - span),
        b = (n.isHold ? n.endSec : n.startSec) + span,
        i0 = Math.floor(a / bs),
        i1 = Math.floor(Math.max(a, b) / bs);
      if (i1 - i0 > 256) {
        gp.longNotes.push(n);
        continue;
      }
      for (let i = i0; i <= i1; i++) {
        let q = gp.timeBuckets.get(i);
        if (!q) gp.timeBuckets.set(i, q = []);
        q.push(n);
      }
    }
  ...
  function gpAwardExactHead(n, e) {
    e.judgeState = 'Exact';
    e.judgeTime = n.startSec;
    e.judgeHited = true;
    e.headJudged = true;
    e.hitParticleOffset = 0;
    e.judgeIsGood = false;
    gpSubmitAcc(1);
    gpCut('Exact');
    gpAddCombo();
  ...
  function gpRebuildExactBefore(target) {
    ...
    gpFresh(rt);
    target = clamp(Number(target) || 0, 0, (rt == null ? void 0 : rt.duration) || 0);
    if (!rt) {
    ...
    for (const n of rt.notes) {
      ...
      if (n.isHold) {
        if (n.endSec < target) {
          ...
          gpAddCombo();
          e.gaveEndCombo = true;
          e.judgeHoldLastCheck = n.endSec;
          e.seekHeld = false;
        } else {
          e.judgeHoldLastCheck = target;
          e.seekHeld = true;
        }
      }
    }
  ...
  function gpEnsure() {
    if (gp.rt !== state.runtime) gpFresh(state.runtime);
  }
  function gpSetAutoplay(on) {
    gp.autoplay = !!on;
    state.autoplay = gp.autoplay;
    gpReleaseInput();
  ...
  function gpTransform(m, a, b, c, d, e, f) {
    return [m[0] * a + m[2] * b, m[1] * a + m[3] * b, m[0] * c + m[2] * d, m[1] * c + m[3] * d, m[0] * e + m[2] * f + m[4], m[1] * e + m[3] * f + m[5]];
  }
  function gpTranslate(m, x, y) {
    return gpTransform(m, 1, 0, 0, 1, x, y);
  }
  function gpScale(m, x, y) {
    return gpTransform(m, x, 0, 0, y, 0, 0);
  }
  function gpRotate(m, deg) {
    const a = Math.PI * deg / 180,
      c = Math.cos(a),
      s = Math.sin(a);
    return gpTransform(m, c, s, -s, c, 0, 0);
  }
  function gpInverse(m) {
    const det = m[0] * m[3] - m[1] * m[2],
      q = det === 0 ? 1e9 : 1 / det;
    return [m[3] * q, -m[1] * q, -m[2] * q, m[0] * q, (m[2] * m[5] - m[3] * m[4]) * q, (m[1] * m[4] - m[0] * m[5]) * q];
  }
  function gpPoint(m, x, y) {
    return {
      x: m[0] * x + m[2] * y + m[4],
      y: m[1] * x + m[3] * y + m[5]
    };
  }
  let gpStageRect = null,
    gpStageRectStamp = -1;
  function gpRefreshStageRect() {
    gpStageRect = els.stage.getBoundingClientRect();
  ...
  function gpCanvasPoint(ev, refresh = false) {
    ...
    if (rotated) return {
      x: Math.trunc(sy * els.stage.width),
      y: Math.trunc((1 - sx) * els.stage.height)
    };
    return {
      x: Math.trunc(sx * els.stage.width),
      y: Math.trunc(sy * els.stage.height)
    };
  }
  function gpEventTime(ev) {
    const base = gpInputTime(),
      stamp = Number(ev == null ? void 0 : ev.timeStamp),
      now = performance.now(),
      age = Number.isFinite(stamp) && Math.abs(now - stamp) < 10000 ? clamp(now - stamp, 0, 120) : 0;
    return clamp(base - age / 1000 * (Number(state.rate) || 1), 0, state.duration || 0);
  }
  function gpInputTime() {
    ...
    if (state.playing) {
      const elapsed = Math.max(0, (performance.now() - (Number(state.lastTick) || performance.now())) / 1000) * (Number(state.rate) || 1);
      return clamp((Number(state.currentTime) || 0) + elapsed, 0, state.duration || 0);
    }
  ...
  function gpGetNotes(t, touch = null) {
    var _gp$timeBuckets, _gp$longNotes;
    const src = ((_gp$timeBuckets = gp.timeBuckets) == null ? void 0 : _gp$timeBuckets.get(Math.floor(Math.max(0, t) / (gp.bucketSec || .25)))) || [],
      out = [];
    for (const n of src) {
      if (!n.isHold && n.startSec - GP_WINDOWS.Bad * 2 <= t && t <= n.startSec + GP_WINDOWS.Bad * 2 || n.isHold && n.startSec - GP_WINDOWS.Bad * 2 <= t && t <= n.endSec + GP_WINDOWS.Bad * 2) out.push(n);
    }
    for (const n of gp.longNotes || []) if (n.startSec - GP_WINDOWS.Bad * 2 <= t && t <= n.endSec + GP_WINDOWS.Bad * 2) out.push(n);
    if ((_gp$longNotes = gp.longNotes) != null && _gp$longNotes.length) out.sort((a, b) => a.startSec - b.startSec || a.globalIdx - b.globalIdx);
    ...
    if (touch != null) out.sort((a, b) => {
      if (Math.abs(a.startSec - b.startSec) > .020) return a.startSec - b.startSec;
      const pa = gpRelTouchPoint(t, a, touch),
        pb = gpRelTouchPoint(t, b, touch);
      return Math.hypot(pa.x, pa.y) - Math.hypot(pb.x, pb.y);
    });
    return out;
  }
  function gpTouchStart(sig, p, isKey = false, eventTime = null) {
    if (!gpIsPlay() || !state.playing || gp.autoplay) return;
    gpEnsure();
    const t = eventTime == null ? gpInputTime() : Number(eventTime);
    let touch = gp.touches.get(sig);
    if (!touch) {
      touch = {
        sig,
        x: p.x,
        ...
        startTime: t,
        isKey: !!isKey,
        holdTouchEnd: gp.touchHoldEnd
      };
      gp.touches.set(sig, touch);
    } else {
      touch.isKey = !!isKey;
      touch.x = p.x;
      touch.y = p.y;
    ...
    for (const n of gpGetNotes(t, touch)) {
      ...
      if (gpIsHit(t, n, touch) && !e.headJudged && n.startSec - GP_WINDOWS.Bad < t && n.type === NOTE_HIT) {
        const offset = t - n.startSec,
          name = gpJudgeState(n, offset);
        e.judgeState = name;
        if (GP_HIT_STATES.has(name)) {
          e.judgeTime = t;
          e.judgeHited = true;
          e.headJudged = true;
          e.hitParticleOffset = offset;
          e.judgeIsGood = name === 'Great' || name === 'Good';
          gp.effectKeys.add(n.key);
          gpCallback(name, offset, t);
          if (n.isHold) {
            e.judgeHoldLastCheck = t;
            gp.touchHoldEnd = Math.max(n.endSec, gp.touchHoldEnd);
          }
          ...
          gpAddCombo();
          gpSubmitAcc(GP_ACC[name]);
          gpCut(name);
          if (n.isHold) gp.holdLasts = n.startSec;
          break;
        }
        if (name === 'Bad' && !n.isHold) {
          ...
          gpCallback('Bad', offset, t);
          e.judgeTime = t;
          e.judgeHited = true;
          e.headJudged = true;
          e.judgeIsMiss = false;
          gpSubmitAcc(GP_ACC.Bad);
          gpCut('Bad');
          gpRemoveCombo();
          break;
        }
  ...
  function gpTouchMove(sig, p, eventTime = null) {
    if (!gpIsPlay() || gp.autoplay) return;
    const touch = gp.touches.get(sig);
    if (!touch) return;
    touch.x = p.x;
    touch.y = p.y;
    gpUpdateAt(eventTime == null ? gpInputTime() : Number(eventTime));
  }
  function gpTouchEnd(sig, p, eventTime = null) {
    if (!gpIsPlay() || gp.autoplay) return;
    const touch = gp.touches.get(sig);
    if (touch && p) {
      touch.x = p.x;
  ...
  window.__gpRebindRuntime = function (rt) {
    const old = gp.rt,
      same = old && old.notes.length === rt.notes.length && old.notes.every((n, i) => {
        const m = rt.notes[i];
        return n.key === m.key && n.startSec === m.startSec && n.endSec === m.endSec && n.type === m.type && n.isFake === m.isFake && n.isAlwaysPerfect === m.isAlwaysPerfect;
      });
    if (!same) {
      gpRebuildExactBefore(state.currentTime);
      return;
    }
  ...
  function gpHoldSustained(n) {
    for (const tc of gp.touches.values()) if (tc.holdTouchEnd >= n.endSec) return true;
    return false;
  }
  ...
  function gpTryPreEndActivate(n, t, e) {
    ...
    for (const touch of gp.touches.values()) {
      ...
      e.judgeTime = t;
      e.judgeState = 'Exact';
      e.hitParticleOffset = Math.max(0, t - n.startSec);
      e.judgeIsGood = false;
      gp.effectKeys.add(n.key);
      gpCallback('Exact', e.hitParticleOffset, t);
      if (n.isHold) {
        e.judgeHoldLastCheck = t;
        gp.touchHoldEnd = Math.max(n.endSec, gp.touchHoldEnd);
        touch.holdTouchEnd = gp.touchHoldEnd;
  ...
  function gpProcessNote(n, t, catchup = false) {
    ...
    for (const touch of gp.touches.values()) {
      if (n.type === NOTE_DRAG && !e.headJudged && gpIsHit(t, n, touch)) {
        const offset = t - n.startSec,
          stateName = gpJudgeState(n, offset);
        if (GP_HIT_STATES.has(stateName)) {
          e.prejudgeTouches.add(touch.sig);
          if (!e.dragPrejudge) {
            e.dragPrejudge = true;
            e.prejudgeTime = t;
          }
    ...
    if (n.type === NOTE_DRAG && !e.headJudged && e.dragPrejudge && n.startSec <= t) {
      e.judgeHited = true;
      e.headJudged = true;
      e.judgeTime = Math.max(n.startSec, t);
      e.judgeState = 'Exact';
      e.hitParticleOffset = e.judgeTime - n.startSec;
      gp.effectKeys.add(n.key);
      gpCallback('Exact', 0, t);
      for (const sig of e.prejudgeTouches) {
        const tc = gp.touches.get(sig);
        if (tc) tc.holdTouchEnd = gp.touchHoldEnd;
      }
      e.prejudgeTouches.clear();
      gpSubmitAcc(1);
      gpCut('Exact');
      gpAddCombo();
    }
    ...
    if (n.isHold && (e.headJudged && !e.judgeIsMiss && t - e.judgeHoldLastCheck >= .05 && n.endSec - .3 >= t || !e.headJudged && t - n.startSec >= GP_WINDOWS.Bad) && !e.judgeIsMiss) {
      ...
      gpSubmitAcc(0);
      gpCut('Miss');
      if (!e.headJudged) gpCallback(e.judgeState, t - n.startSec, t);
    }
    if (catchup && n.isHold && e.headJudged && !e.judgeIsMiss && !e.gaveEndCombo && n.endSec < t && !gpHoldSustained(n)) {
      ...
      e.judgeIsMiss = true;
      e.judgeMissTime = Math.min(t, n.endSec);
      gpRemoveCombo();
      gpSubmitAcc(0);
      gpCut('Miss');
    }
    if (n.isHold && e.headJudged && !e.judgeIsMiss && n.endSec < t && !e.gaveEndCombo) {
      ...
      e.gaveEndCombo = true;
      e.seekHeld = false;
      if (e.judgeState === 'Exact') gpCut('Exact');else if (e.judgeState === 'Perfect') gpCut('Perfect');else if (e.judgeState === 'Great') gpCut('Great');else if (e.judgeState === 'Good') gpCut('Good');
    }
    if (!n.isHold && !e.headJudged && !e.judgeIsMiss && t - n.startSec >= GP_WINDOWS.Bad) {
      if (!e.dragPrejudge) {
        e.judgeIsMiss = true;
        gpCallback(e.judgeState, t - n.startSec, t);
        e.judgeMissTime = t;
        gpSubmitAcc(0);
        gpCut('Miss');
        gpRemoveCombo();
  ...
  function gpSweepSkipped(fromT, toT) {
    ...
    const bs = gp.bucketSec || .25,
      span = GP_WINDOWS.Bad * 2;
    const i0 = Math.floor(Math.max(0, fromT - span) / bs),
      i1 = Math.floor(Math.max(0, toT + span) / bs);
    if (i1 - i0 > GP_SWEEP_MAX_BUCKETS) {
      for (const n of gp.noteByKey.values()) gpProcessNote(n, toT, true);
      return;
    }
    const seen = gp.sweepSeen || (gp.sweepSeen = new Set());
    seen.clear();
    for (const n of gp.longNotes || []) gpProcessNote(n, toT, true);
    for (let i = i0; i <= i1; i++) {
      const q = gp.timeBuckets.get(i);
      if (!q) continue;
      for (const n of q) {
        if (seen.has(n.key)) continue;
        seen.add(n.key);
        gpProcessNote(n, toT, true);
      }
    }
  }
  function gpAdvance(t) {
    if (!gpIsPlay() || gp.autoplay) return;
    gpEnsure();
    if (t < gp.lastFixed - .03) {
      gpRebuildExactBefore(t);
      return;
    }
    ...
    let guard = 0;
    while (gp.lastFixed + GP_STEP < t && guard++ < 12) {
      gpUpdateAt(gp.lastFixed);
      gp.lastFixed += GP_STEP;
    }
    if (gp.lastFixed + GP_STEP < t) {
      gpSweepSkipped(gp.lastFixed, t);
  ...
  function gpProcessScore() {
    return score.process(gpScoreState());
  }
  window.__gpProcessScore = gpProcessScore;
  function gpHudComboLabel() {
    return score.label(gpScoreState(), gp.autoplay);
  }
  window.__gpHudComboLabel = gpHudComboLabel;
  function gpHudMetrics(sec) {
    const st = gpScoreState(sec);
    return {
      score: score.process(st),
      acc: st.len ? st.acc / (st.len * 1000000) : 0,
      combo: st.combo,
      label: score.label(st, gp.autoplay)
    };
  }
  ...
  let gpEffectKind = 'normal',
    gpDrawingRing = false;
  const gpParticleColorBase = typeof __pluParticleColor === 'function' ? __pluParticleColor : null;
  if (gpParticleColorBase) {
    __pluParticleColor = function (p) {
      if (gpEffectKind === 'good') return [133, 255, 189];
      if (gpDrawingRing) return [150, 144, 253];
      const q = clamp(p / .75, 0, 1);
      return [142 + (162 - 142) * q, 197 + (66 - 197) * q, 252 + (255 - 252) * q];
    };
  }
  function gpEffectEntry(n, sec) {
    if (!gpIsPlay() || n.isFake) return null;
    if (n.type === NOTE_FRACTURE) return gp.autoplay && sec >= n.startSec ? {
      allow: true,
      good: false
    } : null;
    if (gp.autoplay) return sec >= n.startSec ? {
    ...
    const e = gpEntry(n);
    if (!e.headJudged || !e.judgeHited) return null;
    if (n.isHold && e.judgeIsMiss && sec >= e.judgeMissTime) return null;
    return {
      allow: true,
      good: e.judgeIsGood
    };
  ...
  if (gpHitRingRaw) {
    __pluDrawHitRing = function (rt, n, sec, st, w, h) {
      const q = gpEffectEntry(n, sec);
      if (gpIsPlay() && !gp.autoplay) return;
      if (gpIsPlay() && !q) return;
      const old = gpEffectKind,
        oldRing = gpDrawingRing;
      gpEffectKind = q != null && q.good ? 'good' : 'normal';
      gpDrawingRing = true;
      try {
        return gpHitRingRaw(rt, n, sec, st, w, h);
      } finally {
        gpEffectKind = old;
        gpDrawingRing = oldRing;
      }
    };
  }
  if (gpParticlesRaw) {
    __pluDrawParticles = function (rt, n, sec, st, w, h) {
      const q = gpEffectEntry(n, sec);
      if (gpIsPlay() && !gp.autoplay) return;
      if (gpIsPlay() && !q) return;
      const old = gpEffectKind;
      gpEffectKind = q != null && q.good ? 'good' : 'normal';
      try {
        return gpParticlesRaw(rt, n, sec, st, w, h);
      } finally {
        gpEffectKind = old;
      }
    };
  ...
  function gpDrawManualEffects(rt, sec, w, h, lineStates = null) {
    ...
    for (const key of gp.effectKeys) {
      ...
      const e = gpEntry(n);
      if (!e.headJudged || !e.judgeHited || sec < e.judgeTime) continue;
      const age = sec - e.judgeTime,
        endFx = n.isHold ? Math.min(n.endSec, e.judgeIsMiss ? e.judgeMissTime : n.endSec) + .5 : e.judgeTime + .5;
      if (sec > endFx) {
        gp.effectKeys.delete(key);
        continue;
      }
      ...
      try {
        if (gpHitRingRaw && age <= .5) {
          gpDrawingRing = true;
          gpHitRingRaw(rt, proxy, sec, st, w, h);
          gpDrawingRing = false;
        }
        if (gpParticlesRaw && (!n.isHold ? age <= .5 : sec <= proxy.endSec + .5)) gpParticlesRaw(rt, proxy, sec, st, w, h);
      } finally {
        gpEffectKind = old;
        gpDrawingRing = oldRing;
      }
  ...
  if (typeof drawCombo === 'function') {
    drawCombo = function (rt, sec, w, h) {
      if (!gpIsPlay()) return;
      const metrics = gpHudMetrics(sec),
        combo = metrics.combo,
        score = metrics.score,
      ...
      ctx.font = `700 ${Math.max(16, w * .0201)}px ui-sans-serif,system-ui`;
      ctx.fillText(metrics.label, w * .5, w * .03594);
      ctx.font = `800 ${Math.max(20, w * .02634)}px ui-sans-serif,system-ui`;
      ctx.fillText(String(combo), w * .5, w * .06771);
      ctx.textAlign = 'right';
      ctx.font = `800 ${Math.max(20, w * .02684)}px ui-monospace,SFMono-Regular,Consolas,monospace`;
      ctx.fillText(String(score).padStart(7, '0'), w * .915, w * .03958);
      ctx.fillStyle = 'rgba(255,255,255,.75)';
      ctx.font = `600 ${Math.max(14, w * .02014)}px ui-sans-serif,system-ui`;
      ctx.fillText(acc, w * .915, w * .06684);
      const now = sec;
      gp.indicatorBalls = gp.indicatorBalls.filter(b => now - b.at <= .52 && now - b.at >= -.05);
      const by = 41.95082 / 1080 * h,
        br = Math.max(4, 25 / 1920 * w);
      for (const b of gp.indicatorBalls) {
        const age = Math.max(0, now - b.at);
        let fade = age < .1 ? age / .1 : age < .3 ? 1 : age < .5 ? 1 - (age - .3) / .2 : 0;
        if (fade <= 0) continue;
        const c = GP_EL_COLORS[b.kind] || GP_EL_COLORS.bad,
          x = w * .5 + w * .2 * b.offset * 6;
        ctx.globalAlpha = .5019608 * fade;
      ...
      ctx.globalAlpha = 1;
      ctx.shadowBlur = 0;
      const progress = rt.duration > 0 ? clamp(sec / rt.duration, 0, 1) : 0;
      ctx.fillStyle = 'rgba(255,255,255,.95)';
      ctx.fillRect(0, 0, w * progress, Math.max(2, w * .0046875));
      ctx.restore();
    };
  }
  ...
  render = function () {
    if (gpIsPlay() && !gpSeeking) gpAdvance(state.currentTime || 0);
    return gpRenderBase();
  };
  ...
  seek = function (t) {
    ...
    if (gpIsPlay()) {
      gpRebuildExactBefore(state.currentTime || 0);
      render();
    }
    return r;
  };
  const gpModeBase = typeof setAppMode === 'function' ? setAppMode : null;
  if (gpModeBase) setAppMode = function (mode) {
    const r = gpModeBase(mode);
    gpReleaseInput();
    if (mode === 'play') {
      gpRebuildExactBefore(state.currentTime || 0);
      render();
  ...
  if (gpPlayBase) setPlaying = function (v) {
    const r = gpPlayBase(v);
    if (!state.playing) gpReleaseInput();
    return r;
  };

  /* Pointer Events map one-for-one to touchstart/move/end. */
  const stage = els.stage;
  function gpLatestPointer(ev) {
  ...
  function gpCaptureDown(ev) {
    ...
    const q = gpLatestPointer(ev);
    gpTouchStart(ev.pointerId, gpCanvasPoint(q, true), false, gpEventTime(q));
  }
  function gpCaptureMove(ev) {
    if (!gpIsPlay() || gp.autoplay || !gp.touches.has(ev.pointerId)) return;
    ev.preventDefault();
    ev.stopImmediatePropagation();
    const q = gpLatestPointer(ev);
    gpTouchMove(ev.pointerId, gpCanvasPoint(q), gpEventTime(q));
  }
  function gpCaptureUp(ev) {
    if (!gpIsPlay() || gp.autoplay || !gp.touches.has(ev.pointerId)) return;
    ev.preventDefault();
    ev.stopImmediatePropagation();
    const q = gpLatestPointer(ev);
    gpTouchEnd(ev.pointerId, gpCanvasPoint(q), gpEventTime(q));
    try {
      stage.releasePointerCapture(ev.pointerId);
    } catch {}
  }
  stage.addEventListener('pointerdown', gpCaptureDown, {
    capture: true,
    passive: false
  });
  /* pointerrawupdate is delivered before render-aligned pointermove on supporting mobile browsers. */
  if ('onpointerrawupdate' in window) stage.addEventListener('pointerrawupdate', gpCaptureMove, {
    capture: true,
    passive: false
  });else stage.addEventListener('pointermove', gpCaptureMove, {
    capture: true,
    passive: false
  });
  stage.addEventListener('pointerup', gpCaptureUp, {
    capture: true,
    passive: false
  });
  stage.addEventListener('pointercancel', gpCaptureUp, {
    capture: true,
    passive: false
  });
  ...
  window.addEventListener('keydown', ev => {
    ...
    if (!gpIsPlay() || gp.autoplay || !state.playing || ev.repeat || !/^(?:Key[A-Z]|Space)$/.test(ev.code)) return;
    const tag = ((_document$activeEleme = document.activeElement) == null ? void 0 : _document$activeEleme.tagName) || '';
    if (['INPUT', 'TEXTAREA', 'SELECT'].includes(tag)) return;
    const id = 'key:' + ev.code;
    if (gpKeys.has(id)) return;
    ev.preventDefault();
    gpKeys.add(id);
    gpTouchStart(id, {
      x: els.stage.width / 2,
      y: els.stage.height / 2
    }, true);
  }, true);
  ...
  window.addEventListener('blur', gpReleaseInput, {
    passive: true
  });
  window.addEventListener('pagehide', gpReleaseInput, {
    passive: true
  });
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) gpReleaseInput();
  }, {
    passive: true
  });
  const autoBox = document.getElementById('autoplayToggle');
  if (autoBox) {
    autoBox.checked = false;
    autoBox.addEventListener('change', () => gpSetAutoplay(autoBox.checked));
  }
  ...
  window.__gpTest = {
    ...
    advance: gpAdvance,
    updateAt: gpUpdateAt,
    touchStart: gpTouchStart,
    touchEnd: gpTouchEnd,
    sweep: gpSweepSkipped,
    release: gpReleaseInput,
