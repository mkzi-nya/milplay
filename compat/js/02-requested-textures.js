...
function __milRegisterUploadedAssetBatch(files) {
  ...
  const rec = {
    id: state.uploadedAssetBatchSerial = (Number(state.uploadedAssetBatchSerial) || 0) + 1,
    files: batch,
    paths: new Map(),
    pathsLower: new Map(),
    names: new Map(),
  ...
  for (const f of batch) {
    const path = __milNormalizePath(f.name);
    if (!path) continue;
    add(rec.paths, path, f);
    add(rec.pathsLower, path.toLowerCase(), f);
    const name = __milBasename(path);
    add(rec.names, name, f);
    add(rec.namesLower, name.toLowerCase(), f);
  }
...
function __milUniqueBatchMatch(map, key) {
  const list = map == null ? void 0 : map.get(key);
...
function __milResolveUploadedAssetFile(ref, baseDir = state.assetBaseDir || '') {
  ...
  const normalized = __milNormalizePath(raw),
    joined = __milJoin(baseDir, raw),
    base = __milBasename(normalized);
  const batches = __milEnsureUploadedAssetBatches();
  for (let i = batches.length - 1; i >= 0; i--) {
    ...
    for (const key of [joined, normalized]) {
      const f = __milUniqueBatchMatch(b.paths, key);
      if (f) return f;
    }
    const exact = __milUniqueBatchMatch(b.names, base);
    if (exact) return exact;
    /* Case-insensitive fallback is accepted only when unique inside that batch. */
    for (const key of [joined.toLowerCase(), normalized.toLowerCase()]) {
      const f = __milUniqueBatchMatch(b.pathsLower, key);
      if (f) return f;
    }
    const folded = __milUniqueBatchMatch(b.namesLower, base.toLowerCase());
...
function __milBatchSuppliesCurrentStoryboard(batch) {
  ...
  const set = new Set(batch.files),
    objects = state.chart.storyboardObjects || state.chart.storyboards || [];
  for (const sb of objects) {
    if (int(sb == null ? void 0 : sb.type, 0) !== 0 || !(sb != null && sb.data) || /^data:|^blob:|^https?:/i.test(sb.data)) continue;
    const f = __milResolveUploadedAssetFile(sb.data);
    if (f && set.has(f)) return true;
  }
  return false;
}
...
window.__milthmFullRenderSelfTest = async function () {
  const result = await __milFullSelfTestBeforeRequestedPatch(),
    failures = [...((result == null ? void 0 : result.failures) || [])],
    ok = (v, m) => {
  ...
  try {
    ...
    const f = new File([new Uint8Array([1])], 'uploaded/story-only.png', {
      type: 'image/png'
    });
    __milRegisterUploadedAssetBatch([f]);
    ...
    const older = new File([new Uint8Array([2])], 'req-old/fallthrough.png', {
      type: 'image/png'
    });
    __milRegisterUploadedAssetBatch([older]);
    const dupA = new File([new Uint8Array([3])], 'req-new-a/fallthrough.png', {
        type: 'image/png'
      }),
      dupB = new File([new Uint8Array([4])], 'req-new-b/fallthrough.png', {
        type: 'image/png'
      });
    __milRegisterUploadedAssetBatch([dupA, dupB]);
