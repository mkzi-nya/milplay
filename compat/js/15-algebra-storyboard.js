(() => {
  ...
  window.__algebraStoryboardReviewSelfTest = function () {
    ...
    try {
      ...
      ok(/^assets\/.+\.(webp|png)$/.test(__ALG_SB_BUILTIN_NOTE_SRC.extap) || /^data:image\/png;base64,/.test(__ALG_SB_BUILTIN_NOTE_SRC.extap), 'dedicated reference builtin note texture');
