(_els$showHandsToggle => {
  ...
  setAppMode = function (mode) {
    ...
    if (mode === 'play') state.showHandTextures = state.__playShowHands === true;else state.showHandTextures = true;
    updateModeUI();
    render();
  };
  (_els$showHandsToggle = els.showHandsToggle) == null || _els$showHandsToggle.addEventListener('change', () => {
    if (state.appMode === 'play') state.__playShowHands = !!els.showHandsToggle.checked;
  });
  ...
  let __targetTintCanvas = null,
    __targetTintCtx = null;
  __milTintSlice = function (img, sx, sy, sw, sh, dx, dy, dw, dh, color) {
    const iw = Number((img == null ? void 0 : img.naturalWidth) || (img == null ? void 0 : img.width)) || 0,
      ih = Number((img == null ? void 0 : img.naturalHeight) || (img == null ? void 0 : img.height)) || 0;
    ...
    const zoom = Math.abs(Number(state.viewScale)) || 1,
      ...
      tintSide = state.lowMemory ? RENDER_QUALITY.lowMemoryTintSide : RENDER_QUALITY.maxTintSide,
      tintPixels = state.lowMemory ? RENDER_QUALITY.lowMemoryTintPixels : RENDER_QUALITY.maxTintPixels,
      scale = Math.min(1, tintSide / Math.max(rw, rh), Math.sqrt(tintPixels / (rw * rh))),
      tw = Math.max(1, Math.floor(rw * scale)),
      th = Math.max(1, Math.floor(rh * scale));
    if (!__targetTintCanvas) {
      __targetTintCanvas = document.createElement('canvas');
      __targetTintCtx = __targetTintCanvas.getContext('2d', {
        alpha: true,
        willReadFrequently: false
      });
    }
    // 先缩小再扩宽，长条横竖切换时也不产生超预算的中间缓冲。
    if (__targetTintCanvas.width !== tw) {
      __targetTintCanvas.height = 1;
      __targetTintCanvas.width = tw;
    }
    if (__targetTintCanvas.height !== th) __targetTintCanvas.height = th;
    const g = __targetTintCtx;
    ...
    g.drawImage(img, sx, sy, sw, sh, 0, 0, tw, th);
    g.globalCompositeOperation = 'source-over';
    ctx.drawImage(__targetTintCanvas, 0, 0, tw, th, dx, dy, dw, dh);
  };
    ...
    if (n.isFake && !n.isHold) {
      ...
      if (decorativeEnd > n.startSec + 1e-6) {
        ...
        if (sec >= n.startSec) {
          ...
          dn = Object.create(n);
          dn.startSec = decorativeEnd;
          dn.endSec = decorativeEnd;
          dn.floorStart = rt.lineValue(n.lineIdx, SPEED, sec);
          dn.floorEnd = dn.floorStart;
        }
  ...
  function __targetIntrinsic(data, img) {
    ...
    try {
      ...
      w = Number((rec == null ? void 0 : rec.sourceWidth) || (rec == null ? void 0 : rec.width) || 0);
      h = Number((rec == null ? void 0 : rec.sourceHeight) || (rec == null ? void 0 : rec.height) || 0);
    } catch {}
    if (!(w > 0)) w = Number((img == null ? void 0 : img.naturalWidth) || (img == null ? void 0 : img.videoWidth) || (img == null ? void 0 : img.width) || 0);
    if (!(h > 0)) h = Number((img == null ? void 0 : img.naturalHeight) || (img == null ? void 0 : img.videoHeight) || (img == null ? void 0 : img.height) || 0);
    return {
      w,
      h
    };
  }
  function __targetTextPx(w, h) {
    return 300 * .1125 * Math.max(w / 800, h / 600);
  }
  function __targetDrawBuiltinPrimitive(kind, cx, cy, dw, dh, deg, color) {
    var _color$, _color$2, _color$3;
    const alpha = Math.max(0, Math.min(1, (Number(color == null ? void 0 : color[3]) || 0) / 255));
    if (alpha <= 0) return;
    ctx.save();
    ctx.translate(cx, cy);
    ctx.rotate((Number(deg) || 0) * Math.PI / 180);
    ctx.scale(dw < 0 ? -1 : 1, dh < 0 ? -1 : 1);
    ctx.globalAlpha *= alpha;
    ctx.fillStyle = `rgb(${(_color$ = color == null ? void 0 : color[0]) != null ? _color$ : 255},${(_color$2 = color == null ? void 0 : color[1]) != null ? _color$2 : 255},${(_color$3 = color == null ? void 0 : color[2]) != null ? _color$3 : 255})`;
    const aw = Math.abs(dw),
      ah = Math.abs(dh),
      x = -aw / 2,
      y = -ah / 2;
    if (kind === 'builtin.round_rect') {
      const r = Math.min(aw, ah) * .18;
      ctx.beginPath();
      ctx.roundRect ? ctx.roundRect(x, y, aw, ah, r) : ctx.rect(x, y, aw, ah);
      ctx.fill();
  ...
  function __targetSbBaseSize(data, intrinsic, w, h) {
    ...
    if (data === 'builtin.tap' || data === 'builtin.tap_double' || data === 'builtin.extap' || data === 'builtin.extap_double') {
      ...
      const ar = (intrinsic == null ? void 0 : intrinsic.w) > 0 && (intrinsic == null ? void 0 : intrinsic.h) > 0 ? intrinsic.h / intrinsic.w : 1;
      return {
        w: fullW,
        h: fullW * ar
      };
    }
    const bw = milX(intrinsic.w, w);
    return {
      w: bw,
      h: bw / intrinsic.w * intrinsic.h
    };
  }
  ...
  function __targetDrawQuad(rt, sb, sec, img, cx, cy, dw, dh, deg, color) {
    ...
    for (const ids of [[0, 2, 1], [1, 2, 3]]) {
      ...
      const u = b[0] - a[0],
        ...
        t = c[1] - a[1],
        det = u * t - s * v;
      const xx = ((B[0] - A[0]) * t - (C[0] - A[0]) * v) / det,
        xy = ((C[0] - A[0]) * u - (B[0] - A[0]) * s) / det;
      const yx = ((B[1] - A[1]) * t - (C[1] - A[1]) * v) / det,
        yy = ((C[1] - A[1]) * u - (B[1] - A[1]) * s) / det;
      ...
      __milTintSlice(img, 0, 0, sw, sh, 0, 0, sw, sh, color);
      ctx.restore();
    }
    ctx.restore();
  }
  drawStoryboardLayer = function (rt, layer, sec, w, h, distorted = false) {
    ...
    for (const sb of list) {
      ...
      const alpha = clamp(rt.sbValue(sb, TRANSPARENCY, sec), 0, 1);
      if (alpha <= 0) continue;
      const drawAlpha = hidden ? Math.min(.45, alpha) : alpha;
      const px = milX(rt.sbValue(sb, POS_X, sec), w),
        py = milY(rt.sbValue(sb, POS_Y, sec), h),
        rx = milX(rt.sbValue(sb, REL_X, sec), w),
        ry = milY(rt.sbValue(sb, REL_Y, sec), h),
        center = localToScreen(w, h, px + rx, py + ry),
        size = Number(rt.sbValue(sb, SIZE, sec)),
        rotDeg = Number(rt.sbValue(sb, ROTATION, sec)),
        sx = Number(rt.sbValue(sb, SB_WIDTH, sec)),
        sy = Number(rt.sbValue(sb, SB_HEIGHT, sec)),
        color = __targetStoryColor(rt, sb, sec, drawAlpha);
      ...
      if (sb.type === 0) {
        ...
        if (sb.distorted) {
          const image = storyImage(data);
          if (!image) continue;
          const intrinsic = __targetIntrinsic(data, image),
            base = data === 'builtin.rect' || data === 'builtin.round_rect' ? {
              w: milX(300, w),
              h: milX(300, w)
            } : __targetSbBaseSize(data, intrinsic, w, h);
        ...
        if (data === 'builtin.rect' || data === 'builtin.round_rect') {
          const baseW = milX(300, w),
            baseH = baseW,
            dw = baseW * size * sx,
            dh = baseH * size * sy,
            rad = .5 * Math.hypot(Math.abs(dw), Math.abs(dh));
        ...
        const img = storyImage(data);
        if (!img) continue;
        const d = __targetIntrinsic(data, img);
        if (!(d.w > 0 && d.h > 0)) continue;
        const base = __targetSbBaseSize(data, d, w, h),
          dw = base.w * size * sx,
          dh = base.h * size * sy,
          rad = .5 * Math.hypot(Math.abs(dw), Math.abs(dh));
        if (center.x + rad < -8 || center.x - rad > w + 8 || center.y + rad < -8 || center.y - rad > h + 8) continue;
        ...
        const text = String(sb.data || ''),
          base = __targetTextPx(w, h);
        ctx.save();
        ctx.translate(center.x, center.y);
        ctx.rotate(-rotDeg * Math.PI / 180);
        ctx.scale(size * sx, size * sy);
        ctx.globalAlpha *= color[3] / 255;
        ctx.fillStyle = `rgb(${color[0]},${color[1]},${color[2]})`;
        ctx.font = `400 ${Math.max(1, base)}px ui-sans-serif,system-ui,"Microsoft YaHei",sans-serif`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(text, 0, 0);
        const mw = Math.max(base * .5, ctx.measureText(text || ' ').width),
          mh = base * 1.15;
        ctx.restore();
        const iw = Math.abs(mw * size * sx),
          ih = Math.abs(mh * size * sy);
        if (hidden) drawHiddenHalo(center.x, center.y, Math.max(iw, ih) / 2);
        if (state.appMode !== 'play') state.inspectHit.push({
          kind: 'storyboard',
          label: '故事板文字',
          id: sb.index,
  ...
  function __targetUnityUiScale(w, h) {
    return Math.max(Math.max(1, w) / 800, Math.max(1, h) / 600);
  }
  drawLineState = function (st, w, h) {
    if (!Number.isFinite(st.absScale) || st.absScale <= 1e-9) return;
    const bodyHidden = isItemHidden('line', st.lineIdx),
      headHidden = isItemHidden('lineHead', st.lineIdx),
      bodyAlpha = st.transparency * st.bodyAlpha,
      headAlpha = st.transparency * st.headAlpha,
      cx = st.center.x,
      cy = st.center.y;
    const ui = __targetUnityUiScale(w, h) * (state.noteScale || 1) * st.absScale,
      headSize = 25.567076 * ui,
      lineWidth = 1.855675 * ui,
      connect = 11.54 * ui,
      bodyLen = 10000 * ui,
    ...
    if (bodyAlpha > .001 && (!bodyHidden || state.showHidden)) {
      const ux = Math.cos(ang),
        ...
        sx = cx + ux * connect,
        sy = cy + uy * connect,
        ex = cx + ux * (connect + bodyLen),
        ey = cy + uy * (connect + bodyLen),
        hw = lineWidth / 2;
      ctx.save();
      ctx.fillStyle = bodyHidden ? 'rgba(255,255,255,.45)' : cssRgba(st.color, bodyAlpha);
      ctx.beginPath();
      ...
      if (state.appMode !== 'play') state.inspectHit.push({
        ...
        label: '判定线',
        id: st.lineIdx,
        hiddenKey: itemKey('line', st.lineIdx),
        x1: sx,
        y1: sy,
        x2: ex,
        y2: ey,
    ...
    if (headAlpha > .001 && (!headHidden || state.showHidden)) {
      const head = imgFor('line_head');
      if (head && head.naturalWidth) __milDrawRotTinted(head, cx, cy, headSize, headSize, 180 - st.rotation, headHidden ? Math.min(.45, headAlpha) : headAlpha, st.color);else {
        ctx.strokeStyle = cssRgba(st.color, headHidden ? Math.min(.45, headAlpha) : headAlpha);
        ctx.lineWidth = Math.max(.25, lineWidth / 2);
        ctx.beginPath();
        ctx.arc(cx, cy, headSize / 2, 0, Math.PI * 2);
        ctx.stroke();
      }
      if (headHidden) drawHiddenHalo(cx, cy, headSize / 2);
      if (state.appMode !== 'play') state.inspectHit.push({
        kind: 'lineHead',
        label: '判定圈',
        id: st.lineIdx,
        hiddenKey: itemKey('lineHead', st.lineIdx),
        x: cx,
        y: cy,
        r: headSize / 2
      });
  ...
  function __targetEarliestAuthoredPreTime(rt, n, base) {
    var _rt$events$get;
    const tracks = (_rt$events$get = rt.events.get(BEARER_NOTE)) == null ? void 0 : _rt$events$get.get(n.animIdx);
    if (!tracks) return base;
    let a = base;
    for (const key of [POS_X, POS_Y, REL_X, REL_Y, FLOW, SIZE, ROTATION, TRANSPARENCY]) {
      var _tr$events;
      const tr = tracks.get(key);
      if (!(tr != null && (_tr$events = tr.events) != null && _tr$events.length)) continue;
      const first = tr.events[0].startSec;
      if (Number.isFinite(first) && first < n.startSec && first < a) a = Math.max(0, first - .05);
    }
    return a;
  }
  function __targetRebuildActiveIndex(rt) {
    rt.__pluLayerNotes = [[], [], []];
    const dur = Math.max(0, Number(rt.duration) || 0);
    ...
    for (const n of rt.notes) if (n.isFake && !n.isHold) {
      let q = fakePerLine.get(n.lineIdx);
      if (!q) fakePerLine.set(n.lineIdx, q = []);
      q.push(n);
    }
    for (const [li, q] of fakePerLine) {
      var _rt$events$get2, _tr$events2;
      if (q.length !== 1) continue;
      const n = q[0],
        tr = (_rt$events$get2 = rt.events.get(BEARER_LINE)) == null || (_rt$events$get2 = _rt$events$get2.get(li)) == null ? void 0 : _rt$events$get2.get(WHOLE_ALPHA);
      if (!(tr != null && (_tr$events2 = tr.events) != null && _tr$events2.length)) continue;
      const after = tr.events.filter(e => e.startSec > n.startSec + 1e-6),
        hasLaterVisible = after.some(e => Math.max(Number(e.fv) || 0, Number(e.tv) || 0) > .001);
      if (!hasLaterVisible) continue;
      const last = tr.events[tr.events.length - 1];
      if (last.startSec > n.startSec + .18) n.__targetDecorativeFakeEnd = Math.min(dur, last.endSec || last.startSec);
    }
    for (const n of rt.notes) {
      var _rt$__pluLayerNotes$l;
      const layer = noteLayer(n);
      (_rt$__pluLayerNotes$l = rt.__pluLayerNotes[layer]) == null || _rt$__pluLayerNotes$l.push(n);
      // No fixed lookahead: negative/zero speed and authored positions can be visible at any time.
      if (n.isFake && !n.isHold) {
        const e = __targetDecorativeFakeEnd(rt, n);
        n.activeTo = e > n.startSec + 1e-6 ? Math.max(n.activeTo, e) : Math.min(n.activeTo, n.startSec);
      }
    }
    rt.notes.forEach((n, i) => {
      n.__drawOrder = i;
    });
    function tree(notes, lo = 0, hi = notes.length) {
      var _left$maxEnd, _right$maxEnd;
      if (lo >= hi) return null;
      const mid = lo + hi >> 1,
        note = notes[mid],
        left = tree(notes, lo, mid),
        right = tree(notes, mid + 1, hi);
      return {
        note,
        left,
        right,
        maxEnd: Math.max(note.activeTo, (_left$maxEnd = left == null ? void 0 : left.maxEnd) != null ? _left$maxEnd : -Infinity, (_right$maxEnd = right == null ? void 0 : right.maxEnd) != null ? _right$maxEnd : -Infinity)
      };
    }
  ...
  window.__targetedReviewSelfTest = function () {
    ...
    try {
      var _state$runtime, _document$getElementB, _els$restoreBar;
      const anchor = (_state$runtime = state.runtime) == null || (_state$runtime = _state$runtime.notes) == null ? void 0 : _state$runtime.find(n => n.isFake && !n.isHold && __targetDecorativeFakeEnd(state.runtime, n) > n.startSec);
      if (anchor) ok(anchor.activeTo >= __targetDecorativeFakeEnd(state.runtime, anchor) - 1e-6, 'decorative fake head indexed through reuse window');
      const drag = {
        type: NOTE_DRAG,
        isMore: true,
      ...
      ok(noteTextureKey(drag) === 'drag_double', 'AP Drag uses ordinary drag_double');
      const plainDrag = {
        type: NOTE_DRAG,
        isMore: false,
      ...
      ok(noteTextureKey(plainDrag) === 'drag', 'Drag ignores hand order');
      const b = __targetSbBaseSize('builtin.extap', {
        w: 923,
        h: 923
      }, 1280, 720);
      ok(b.w > 350 && b.w < 370, 'builtin extap uses 300-Mil visible storyboard footprint');
      ok(Math.abs(b.w * .5 * (510 / 923) - 100) < 1, 'Algebra 188s Size=.5 extap has 100px visible diameter at 1280x720');
      const l = __targetSbBaseSize('builtin.line', {
        w: 512,
        h: 1
      }, 1280, 720);
      ok(l.w > 6000 && l.h > 2, 'builtin line effectively unbounded');
      if (String(((_document$getElementB = document.getElementById('songTitle')) == null ? void 0 : _document$getElementB.textContent) || '').toLowerCase().includes('algebra')) {
        ...
        const transient = state.runtime.notes.find(n => n.isFake && !n.isHold && n.lineIdx === 0);
        if (transient) ok(__targetDecorativeFakeEnd(state.runtime, transient) <= transient.startSec + 1e-6, 'main-line transient fake tap is not persisted');
      }
      ...
      ok(!((_els$restoreBar = els.restoreBar) != null && _els$restoreBar.querySelector('button')), 'obsolete restore/ignore buttons removed');
