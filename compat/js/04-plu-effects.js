...
normalizeMilthm = function (raw) {
  const out = __pluNormalizeMilthmV1(raw);
  if (out && out._milthm && Array.isArray(out.animations)) {
    for (const a of out.animations) {
      const s = (a == null ? void 0 : a._source) || {};
      if (Object.prototype.hasOwnProperty.call(s, 'IsCustomEase')) a.isCustomEase = !!s.IsCustomEase;
      if (Array.isArray(s.CustomEaseArr)) a.customEaseArr = [...s.CustomEaseArr];
      if (Object.prototype.hasOwnProperty.call(s, 'isCustomEase')) a.isCustomEase = !!s.isCustomEase;
      if (Array.isArray(s.customEaseArr)) a.customEaseArr = [...s.customEaseArr];
    }
  }
...
precompute = function (rt) {
  __pluPrecomputeV2(rt);
  if (typeof markStageResize === 'function') markStageResize();
  rt.__pluNonHoldStarts = rt.notes.filter(n => !n.isHold && !n.isFake && n.type !== NOTE_FRACTURE).sort((a, b) => a.startSec - b.startSec);
  rt.__pluHolds = rt.notes.filter(n => n.isHold && !n.isFake).sort((a, b) => a.startSec - b.startSec);
   ...
   * full 8/10 sparks. */
  {
    ...
    for (let i = 0; i < arr.length; i++) {
      const t = arr[i].startSec;
      while (lo < i && arr[lo].startSec < t - .5) lo++;
      while (hi < arr.length && arr[hi].startSec <= t + .5) hi++;
      const active = hi - lo;
      arr[i].__pluParticleStride = active <= 24 ? 1 : active <= 48 ? 2 : 3;
    }
  }
  /* Active Hold hit-effects: avoid scanning every Hold from song start on every frame. */
  rt.__pluLongFxHolds = [];
  rt.__pluHoldFxBuckets = new Map();
  for (const n of rt.__pluHolds) {
    const i0 = Math.floor(Math.max(0, n.startSec)),
      i1 = Math.floor(Math.max(0, n.endSec + .5));
    if (i1 - i0 > 64) {
      rt.__pluLongFxHolds.push(n);
      continue;
    }
    for (let i = i0; i <= i1; i++) {
      let q = rt.__pluHoldFxBuckets.get(i);
      if (!q) rt.__pluHoldFxBuckets.set(i, q = []);
      q.push(n);
    }
  }
};
...
function __pluActiveNotesAt(rt, layer, sec) {
  var _rt$__pluLayerNotes, _rt$chart;
  if (!rt.__activeTrees) return ((_rt$__pluLayerNotes = rt.__pluLayerNotes) == null ? void 0 : _rt$__pluLayerNotes[layer]) || [];
  const out = rt.__activeScratch[layer];
  out.length = 0;
  function visit(node) {
    if (!node || node.maxEnd < sec) return;
    visit(node.left);
    if (node.note.activeFrom <= sec) {
      if (node.note.activeTo >= sec) out.push(node.note);
      visit(node.right);
    }
  }
  visit(rt.__activeTrees[layer]);
  // Preserve authored draw order, including overlapping alpha sprites.
  if ((_rt$chart = rt.chart) != null && _rt$chart._rwc) out.sort((a, b) => a.__drawOrder - b.__drawOrder);
  return out;
}
function __pluTweenGet(anim, time, combo) {
  const p = clamp((time - anim.last) / .15, 0, 1);
  return combo ? 1 + .07 * Math.sin(p * Math.PI) : anim.from + (anim.to - anim.from) * p;
}
function __pluTweenSet(anim, time, target, combo) {
  if (anim.to !== target) {
    const current = __pluTweenGet(anim, time, combo);
    anim.from = combo ? 1 : current;
    anim.to = target;
    anim.last = time;
  }
  return __pluTweenGet(anim, time, combo);
}
drawCombo = function (rt, sec, w, h) {
  const combo = rt.comboAt(sec),
    total = rt.comboTimes.length,
    targetScore = total ? clamp(Math.ceil(1010000 / total * combo), 0, 1010000) : 1010000;
  if (!rt.__pluHud) rt.__pluHud = {
    lastTime: sec,
    score: {
      from: 0,
      to: 0,
  ...
  hud.lastTime = sec;
  const score = Math.max(0, Math.floor(__pluTweenSet(hud.score, sec, targetScore, false))),
    comboScale = __pluTweenSet(hud.combo, sec, combo, true),
    scoreText = String(score).padStart(7, '0');
  ctx.save();
...
render = function () {
  ...
  if (rt) {
    var _rt$__pluHoldFxBucket;
    const sec = clamp(state.currentTime, 0, rt.duration),
      play = state.appMode === 'play';
    ctx.save();
    ...
    const nonholds = rt.__pluNonHoldStarts || [],
      nhStart = __pluLowerByStart(nonholds, sec - .5);
    for (let i = nhStart; i < nonholds.length && nonholds[i].startSec <= sec; i++) __pluDrawHitRing(rt, nonholds[i], sec, lineStates[nonholds[i].lineIdx], w, h);
    const holds = rt.__pluHolds || [],
      hRingStart = __pluLowerByStart(holds, sec - .5);
    ...
    if (!play) {
      state.visibleHit = state.visibleHit.map(v => ({
        ...v,
        ...chartToScreen(v.x, v.y),
        cx: v.x,
        cy: v.y,
        r: v.r * state.viewScale
      }));
      state.inspectHit = state.inspectHit.map(screenMapHit);
    } else {
      state.visibleHit.length = 0;
      state.inspectHit.length = 0;
    }
  }
  drawOverlay(w, h);
};
...
function __pluLowerByStart(arr, value) {
  let lo = 0,
    hi = arr.length;
  while (lo < hi) {
    const mid = lo + hi >> 1;
    if (arr[mid].startSec < value) lo = mid + 1;else hi = mid;
  }
  return lo;
}
