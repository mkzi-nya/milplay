(() => {
  ...
  const isTextInput = () => {
    ...
    return ['INPUT', 'TEXTAREA', 'SELECT'].includes(((_document$activeEleme = document.activeElement) == null ? void 0 : _document$activeEleme.tagName) || '');
  };
  window.addEventListener('keydown', e => {
    if (state.appMode !== 'play' || !state.runtime || isTextInput()) return;
    const result = document.getElementById('gameResultOverlay'),
      resultShown = !!(result != null && result.classList.contains('show'));
    if (e.key === 'Escape') {
      e.preventDefault();
      ...
      if (resultShown) {
        ...
        Promise.resolve(window.__gpLeaveGameplayFullscreen == null ? void 0 : window.__gpLeaveGameplayFullscreen()).catch(() => {});
        return;
      }
      if (state.playing) {
