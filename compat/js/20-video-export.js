(() => {
  ...
  function bestMime() {
    const list = ['video/webm;codecs=vp9,opus', 'video/webm;codecs=vp8,opus', 'video/webm', 'video/mp4;codecs=avc1.42E01E,mp4a.40.2', 'video/mp4'];
    return list.find(x => window.MediaRecorder && (MediaRecorder.isTypeSupported == null ? void 0 : MediaRecorder.isTypeSupported(x))) || '';
  }
  ...
  async function exportVideo() {
    ...
    if (((_state = state) == null ? void 0 : _state.appMode) !== 'play') {
      setStatus('请切换到游玩模式后导出视频。', 'warn');
      return;
    }
    if (!((_state2 = state) != null && _state2.runtime)) {
      setStatus('请先加载谱面。', 'err');
      return;
    }
    if (!HTMLCanvasElement.prototype.captureStream || !window.MediaRecorder) {
      setStatus('当前浏览器不支持 Canvas 视频导出（需要 MediaRecorder + captureStream）。', 'err');
      return;
    }
    const duration = Number(state.duration || state.runtime.duration || 0);
    if (!Number.isFinite(duration) || duration <= 0) {
      setStatus('谱面时长无效，无法导出视频。', 'err');
      return;
    }
    ...
    const saved = {
      ...
      rate: state.rate,
      audioRate: els.audioPlayer.playbackRate,
      showHands: state.showHandTextures,
      chart: state.chart
    };
    job = {
      cancelled: false
    ...
    try {
      ...
      state.rate = 1;
      els.audioPlayer.playbackRate = 1;
      seek(0);
      ...
      if (state.mediaUrl && state.mediaReady && capture) {
        try {
          const as = capture.call(media);
          for (const t of as.getAudioTracks()) tracks.push(t);
        } catch (e) {
          console.warn('[Milthm export] audio capture unavailable', e);
        }
      }
      ...
      const vbr = Math.max(6000000, Math.round(pixels * 12));
      ...
      const stopped = new Promise(resolve => {
        ...
        recorder.addEventListener('error', e => {
          recorderError = e.error || new Error('视频编码失败');
          resolve();
        }, {
          once: true
        });
      ...
      while (!job.cancelled && state.currentTime < duration - 0.002) {
        ...
        if (state.currentTime > lastTime + .001) {
      ...
      setStatus('视频导出失败：' + ((e == null ? void 0 : e.message) || String(e)), 'err');
    } finally {
      try {
      ...
      els.audioPlayer.playbackRate = saved.audioRate || saved.rate || 1;
      state.showHandTextures = saved.showHands;
      if (state.chart === saved.chart) {
        seek(Math.min(saved.time, state.duration || saved.time));
        state.lastTick = performance.now();
