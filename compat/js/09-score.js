(function (root) {
  ...
  function process(st) {
    const N = st.noteAmount,
      n = st.len;
    if (!N || !n) return 0;
    return Math.floor(st.acc / N * (0.4 + 0.6 * (st.procCombo / (n * st.bMax)))) + Math.floor(5000 * st.maxCombo / N) + Math.floor(st.allEP ? 5000 * n / N : 0);
  }
  ...
  function final(st) {
    const N = st.noteAmount;
    if (!N || !st.len) return 0;
    return Math.floor(st.acc / N * (0.4 + 0.6 * (finalCombo(st) / (N * st.bMax)))) + Math.floor(5000 * st.maxCombo / N) + (st.allEP ? 5000 : 0);
  }
  function snapshot(st, complete = st.len === st.noteAmount) {
    const totalComboScore = finalCombo(st);
    return {
      noteAmount: st.noteAmount,
      judged: st.len,
      complete,
  ...
  if (typeof module === 'object' && module.exports) module.exports = api;else root.MilScore = api;
})(globalThis);
